...
public class XMLizacao {
	...
	public static void main(String[] args) {
		...
		String regexLeis = "(( Decreto| \\QDecreto-Lei\\E| Decreto Lei| Decreto Lei Federal| Decreto-Lei Federal| Lei Federal| Lei Complementar| Lei Complementar Federal| Lei| ResoluÁ„o| Emenda Constitucional| Ad Referendum| OfÌcio| Parecer| Parecer Conjunto| Portaria Conjunta| Portaria| Ato( DeclaratÛrio)?| Termo| Ordem Banc·ria| Ajuste| Ordens Banc·rias| Di·rio Oficial (do Estado|da Uni„o))([A-Z \\Q/-\\E]{0,16})((( n∫| n∞)? (\\d{1,2}\\.\\d{3}|\\d{3}|\\d{2}|\\d{2,4}/\\d{2,4}[A-Z\\Q	/ -\\E]{0,10}))?(,)?((( de| em)? (\\d{1,2})(∞|∫)?( de |[\\Q./-\\E])([a-zÁA-Z«]{4,9}|\\d{1,2}[\\Q./-\\E]))?( de )?(\\d{4}))?)( da ConstituiÁ„o Estadual)?)(,|\\Q.\\E|;|\\Q:\\E| [a-z]|\\))";
...
//[0-9A-Za-z\\QÁ„‡·‚ÈÍÌÛÙı˙¬√¡¿… Õ”‘’⁄«&\\E]*
		...
		String regexOrgao = "(( FundaÁ„o| EscritÛrio| ”rg„o| OrganizaÁ„o| Secretaria(s)?| Instituto| Departamento| Procuradoria| AgÍncia| Comiss„o| Junta| Assembleia| C‚mara| ConsÛrcio| Defensoria| Tribunal| Companhia| Controladoria| Gabinete| GerÍncia| ComitÍ| Contadoria| InteligÍncia| Unidade| Grupo| Grupamento| Corpo| Central| Conselho| Banco| PolÌcia| Diretoria| Receita| Delegacia| Autarquia) ((e )?(de |do |dos |da |das |‡ |a |ao |aos |pelo |pelos |pelas |pela |por |em |no |nos |nas |na |para )?[A-Z¬¡… Õ”‘⁄]([A-Z¬√¡… Õ”‘’⁄«]+|[a-zÁ„‡·‚ÈÍÌÛÙı˙]+)(,| )?)+(\\s*(-|/)\\s*[A-Z][A-Za-z\\QÁ„‡·‚ÈÍÌÛÙı˙¬√¡¿… Õ”‘’⁄«&\\E]+)*)(,|,<|\\Q.\\E|;|:| [a-z])";
		String regexProgProj = "(( Regime(s)?| Simples| Projeto(s)?| Programa(s)?| Fundo(s)?| PolÌtica(s)?| GratificaÁ(„o|ıes)| ConsolidaÁ(„o|ıes)| Sistema(s)?| Plano(s)?| Regulamento(s)?| ConvÍnio(s)?| Financiamento(s)?| OrÁamento(s)?| Quadro(s)?) ((e )?(de |do |dos |da |das |‡ |a |ao |aos |pelo |pelos |pelas |pela |por |em |no |nos |nas |na |para )?[A-Z¬¡¿… Õ”‘⁄]([A-Z¬√¡… Õ”‘’⁄«]+|[a-zÁ„‡·‚ÈÍÌÛÙı˙]+)[\\Q-., \\E]{0,3})+(\\s*(-|/)\\s*[A-Z][A-Za-z\\QÁ„‡·‚ÈÍÌÛÙı˙¬√¡¿… Õ”‘’⁄«&\\E]+)*)(,|\\Q.\\E|;|:|<|[a-z]| [a-z])";
		...
		String regexMunicipio = "( (M|m)unicÌpio)( [A-Z¬¡¿… Õ”‘⁄][A-Za-zÁ„‡·‚ÈÍÌÛÙı˙¬√¡¿… Õ”‘’⁄«]+)";
		String regexCargo = "(( Comandante( Geral)?| Chefe| Sargent(o|a)| Tenente| Secret·ri(o|a)| PresidÍncia| Diretor(\\-Presidente|\\-Geral)?| Agente(s)?)( (e )?(de |do |dos |da |das |‡ |a |ao |aos |pelo |pelos |pelas |pela |por |em |no |nos |nas |na |para )?[A-Z¬¡… Õ”‘⁄]([A-Z¬√¡… Õ”‘’⁄«]+|[a-zÁ„‡·‚ÈÍÌÛÙı˙]+)[\\Q-., \\E]{0,3})+)(,|\\Q.\\E|;|:| [a-z])";
		String regexCargo2 = "( Delegad(o|a)(s)?| Escriv(„o|„es)| Agente(s)?| Diretor(es|a|as)?)(,| [a-z ]{1,8}[^A-Z]|\\.)";
			...
			for (int j = 0; j < arquivos[i].length; j++){
				...
				try {
					...
					while (matcher.find() && (matcher.group(7) != null || matcher.group(11) != null || matcher.group(12) != null)){
						...
						textoModificado+= ">" + (matcher.group(11) != null? matcher.group(1).trim(): matcher.group(1).replace(",", "").trim()) + "</" + tiposDocumentos.get(matcher.group(2).trim()) + ">";//+ (matcher.group(10) != null? "": ",") + (matcher.group(19) == matcher.group(19).toLowerCase()? " " : "");
						decretoSaida = decretoSaida.replaceFirst("\\Q"+entreTags+"\\E", textoModificado);
					}
					...
					while (matcher.find()){
						entreTags = matcher.group(3);
						textoModificado = " <MUN>" + matcher.group(3).trim() + "</MUN>";
						decretoSaida = decretoSaida.replaceFirst("\\Q" + entreTags +"\\E", textoModificado);
					}
