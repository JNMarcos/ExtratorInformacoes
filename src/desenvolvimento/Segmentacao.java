...
public class Segmentacao {
			...
			for (int j = 0; j < arquivos[i].length; j++){
				...
				try {
					...
					while (matcher.find() && (matcher.group(7) != null  || matcher.group(9) != null)){ //&& (matcher.group(6) != null || matcher.group(10) != null || matcher.group(11) != null)){
						...
						} else if (matcher.group(6) != null && matcher.group(15) != null){ // 12 È dia, 15 È mÍs, 17 ano
	...
	public static String tabelaReformulada (String tabela){
		...
		matcher = Pattern.compile("<(AT_PROJ|ID_ORG|ID_ORG_G)>([A-Z¡…Õ”⁄¬ ‘√’«¿‡Áa-z·ÈÌÛ˙‚ÍÙ„<>,0-9/ -.]+)</(AT_PROJ|ID_ORG|ID_ORG_G)>\\s*<(VAL|FON)>[0-9.,]+</(VAL|FON)>\\s+([A-Z¡…Õ”⁄¬ ‘√’«Áa-z·ÈÌÛ˙‚ÍÙ„ı,/ -.]+)<").matcher(tabela);
		while (matcher.find()){
		...
		matcher = Pattern.compile("</TIPO>:(\\s*[A-Z¡…Õ”⁄¬ ‘√’«¿‡Áa-z·ÈÌÛ˙‚ÍÙ„ı<>,0-9/ -_.]+\\s*<DESCR_OGM>[A-Z¡…Õ”⁄¬ ‘√’«¿‡Áa-z·ÈÌÛ˙‚ÍÙ„ı<>,0-9/ -.]+</DESCR_OGM>\\s*)(TOTAL|<DESCR_PROJ>)").matcher(tabela);
		while (matcher.find()){
